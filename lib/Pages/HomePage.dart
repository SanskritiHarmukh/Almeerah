  ...
  Future<void> _fetchUserData() async {
    try {
      ...
      if (userSnapshot.exists) {
        ...
        if (userData != null && userData.containsKey('firstname') && userData.containsKey('gender')) {
          ...
          print('Error: Missing required fields (firstname or gender) in user data');
        }
      } else {
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              SizedBox(height: 4,),
              Text(
                _userName.isNotEmpty ? _userName : 'User name not available',
              ...
              Container(
                ...
                child: ListView(
                  ...
                  children: [
                    CategoryContainer(
                        categoryName: 'OOTD', icon: Icons.calendar_month_outlined, pageName: '/ootd'),
                    CategoryContainer(
                        categoryName: 'Fashion Tips',
                        icon: Icons.tips_and_updates_outlined,
                        pageName: '/fashion'),
                    CategoryContainer(
                        categoryName: 'Zodiac Chic', icon: Icons.balance_outlined, pageName: '/zodiac'),
                    CategoryContainer(
                        categoryName: 'NGO Connect', icon: Icons.handshake_outlined, pageName: '/ngo'),
                    CategoryContainer(
                        categoryName: 'Faves', icon: Icons.favorite_border_outlined, pageName: '/fav'),
                    CategoryContainer(
                        categoryName: 'My Closet', icon: Icons.table_chart_outlined, pageName: '/closet'),
                  ],
                ),
              ),
              SizedBox(height: 16,),
                  ...
                  GestureDetector(
                    onTap: () {
                      Navigator.push(
                        context,
                        MaterialPageRoute(
                          builder: (BuildContext context) => RecommendationsPage(
                            gender: _userGender,
                            userID: _user.uid,
                          ),
                        ),
              ...
              SizedBox(height: 8,),
              Expanded(
                child: _userGender.isNotEmpty
                    ? CustomGrid(gender: _userGender, userID: _user.uid)
                    : CircularProgressIndicator(), // or replace with another loading indicator
              ),
            ],
          ),
        ),
